...
export const fetchSessionByDate = async (sessionDate: string): Promise<Session | null> => {
  const sessions = getSessions();
  const match = Object.values(sessions).find((session) => session.sessionDate === sessionDate);
  if (!match) {
