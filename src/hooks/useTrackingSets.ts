...
export const useTrackingSetsForEntry = (exerciseEntryId: string | null) =>
  useQuery({
    ...
    queryFn: () =>
      exerciseEntryId ? fetchTrackingSetsForEntry(exerciseEntryId) : Promise.resolve([]),
...
export const useCreateTrackingSet = () => {
  ...
  return useMutation({
    ...
    onSuccess: (set) => {
      queryClient.invalidateQueries({ queryKey: ["trackingSets", set.exerciseEntryId] });
    },
  });
};
...
export const useUpdateTrackingSet = () => {
  ...
  return useMutation({
    ...
    onSuccess: (_set, variables) => {
      queryClient.invalidateQueries({ queryKey: ["trackingSets", variables.exerciseEntryId] });
    },
  });
};
...
export const useDeleteTrackingSet = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ setId, exerciseEntryId: _exerciseEntryId }: { setId: string; exerciseEntryId: string }) =>
      deleteTrackingSet(setId),
